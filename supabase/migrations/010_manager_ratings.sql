-- SupportOps Module 10: Manager rating columns for 5-point daily scoring
-- Adds manager rating fields to support logs and testing logs

-- Add manager rating columns to daily_support_logs
do $$
begin
  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'daily_support_logs' and column_name = 'ticket_rating'
  ) then
    alter table public.daily_support_logs
      add column ticket_rating integer check (ticket_rating is null or ticket_rating between 1 and 5);
  end if;

  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'daily_support_logs' and column_name = 'chat_rating'
  ) then
    alter table public.daily_support_logs
      add column chat_rating integer check (chat_rating is null or chat_rating between 1 and 5);
  end if;

  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'daily_support_logs' and column_name = 'documentation_rating'
  ) then
    alter table public.daily_support_logs
      add column documentation_rating integer check (documentation_rating is null or documentation_rating between 1 and 5);
  end if;
end $$;

-- Add task_completion column to daily_testing_logs
do $$
begin
  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'daily_testing_logs' and column_name = 'task_completion'
  ) then
    alter table public.daily_testing_logs
      add column task_completion integer not null default 5 check (task_completion between 1 and 5);
  end if;
end $$;

-- Recreate the RPC function with manager ratings
