-- Fix: handle empty strings in RPC testing entry fields
-- The coalesce() only catches null, not empty strings.
-- nullif() converts empty strings to null first, then coalesce applies the default.
